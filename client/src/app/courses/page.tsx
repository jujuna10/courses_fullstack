    ...
    <div className='bg-gradient-to-b from-[#111015] via-[#0c0c18] to-[#07081a] relative overflow-hidden w-full h-screen p-5 flex flex-col gap-36'>
      ...
      <div>
        <input
          ...
          className='w-full py-4 px-5 rounded-[20px] border-[1px] text-white border-blue-500 focus:border-[2px] focus:border-blue-700 bg-gray-500 bg-transparent focus:outline-none'
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearchResult(e.target.value)}
        />
      </div>
      <div className='grid grid-cols-3 gap-12 w-full ml-[3%]'>
        {filtered().map((course, index) => (
          <div
            key={index}
            className={`flex gap-5 w-[80%] py-4 px-5 border-[1px] border-gray-200 rounded-[10px] ${
              course.category === 'Development'
                ? 'shadow-[0px_0px_10px_rgb(255,255,20)]'
                : course.category === 'Design'
                ? 'shadow-[0px_0px_10px_rgb(0,0,255)]'
                : 'shadow-[0px_0px_10px_rgb(255,255,255)]'
            }`}
          >
            <div className='w-full flex flex-col gap-5'>
              ...
              <div className='flex justify-center'>
                <button
                  className='text-white hover:bg-blue-500 transition-all duration-200 w-[35%] px-5 rounded-[10px]'
                  onClick={() => {
                    indexx(course.id);
                    setSureOpen(!sureOpen);
                  }}
                >
                  Buy Course
                </button>
        ...
        <div className='absolute top-[0%] left-[0%] w-full h-screen bg-black bg-opacity-60 z-50 flex justify-center items-center'>
          <div className='flex flex-col gap-5 justify-center items-center'>
            ...
            <div className='flex gap-5 items-center'>
              <button
                className='text-white hover:bg-white hover:text-black px-2 py-1 rounded-[5px]'
                onClick={() => {
                  setSureOpen(false);
                  setSureAnswer('yes');
                  send();
